...
fn max_audio_bytes_defaults_to_the_provider_limit() {
    ...
    let saved_pdf_split = std::env::var("IRONFLOW_MAX_PDF_SPLIT_PAGES").ok();
    unsafe {
        ...
        std::env::remove_var("IRONFLOW_MAX_PDF_SPLIT_PAGES");
    }
    ...
    let pdf_split_pages = ironflow::util::limits::max_pdf_split_pages();
    ...
    unsafe {
        ...
        if let Some(previous) = saved_pdf_split {
            std::env::set_var("IRONFLOW_MAX_PDF_SPLIT_PAGES", previous);
        }
    }
    ...
    assert_eq!(pdf_split_pages, 1_000);
}

#[test]
fn xlsx_ceilings_default_when_unset() {
    // Only defaults are asserted here (no override) -- clear all variables
    ...
    let saved_metadata = std::env::var("IRONFLOW_MAX_XLSX_ARCHIVE_METADATA_BYTES").ok();
    unsafe {
        ...
        std::env::remove_var("IRONFLOW_MAX_XLSX_ARCHIVE_METADATA_BYTES");
    }
    ...
    let metadata_bytes = ironflow::util::limits::max_xlsx_archive_metadata_bytes();
    ...
    unsafe {
        ...
        if let Some(previous) = saved_metadata {
            std::env::set_var("IRONFLOW_MAX_XLSX_ARCHIVE_METADATA_BYTES", previous);
        }
    }
    ...
    assert_eq!(metadata_bytes, 8 * 1024 * 1024);
}
