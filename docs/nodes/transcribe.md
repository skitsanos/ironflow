...
| `temperature`  | number | no          | --                  | Sampling temperature forwarded to the provider. Read as a plain number; not interpolated.                                                                                            |
...
| `timeout`      | number | no          | `120`               | Request timeout in seconds. Must be greater than zero.                                                                                                                               |

The audio file is read from disk before it is uploaded, capped at `IRONFLOW_MAX_AUDIO_BYTES` (default 25,000,000 bytes / 25 MB). A file over the cap fails the step before any network request is made; see [CLI_REFERENCE.md](../CLI_REFERENCE.md) for the environment variable.
