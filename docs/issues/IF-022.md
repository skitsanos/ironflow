...
### 2026-09-16 CI fixture follow-up

The full-feature job on PR #188 exposed a wall-clock assumption in
`backoff_timeout_keeps_the_last_completed_attempt_diagnostics`: a loaded runner
can consume its 30 ms budget before the first attempt executes, correctly
leaving no completed-attempt output to assert against. This in-memory regression
now starts with Tokio time paused, allowing the first failure to complete before
virtual time advances through the retry deadline. Tokio's `test-util` feature is
enabled only as a development dependency. Timeout, attempt-count, task-output,
and final-context assertions are unchanged; production timeout behavior is not
modified. Validation is recorded on the PR after the follow-up gates complete.
