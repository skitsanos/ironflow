...
### 2026-09-16 SDK compatibility follow-up

PR #188 updates `rmcp` to 3.4.0. Its deprecated `ClientInfo` alias failed the
warnings-denied build, so IronFlow now uses `ClientConfig` and declares 3.4.0
as the minimum SDK version. Both aliases represent the same initialization
request; the public `params.clientInfo` field, stable protocol version, and
workflow session contract are unchanged. The HTTP fixture also checks the
configured client identity on the wire.

The focused stdio, Streamable HTTP, and SSE suite passed 18 tests locally.
Full integration and hosted checks are verified separately at the merge boundary.
