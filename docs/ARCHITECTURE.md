...
Nodes are registered in a `NodeRegistry` and exposed to Lua as callable factory functions. 69 built-in nodes are provided across HTTP, shell, file, S3, data transform, iteration, caching, conditional, timing, code execution, markdown, document extraction, image, database, AI, subworkflow, and utility categories. The `pdf_to_image` and `pdf_thumbnail` nodes require the native `pdfium` library at runtime.
