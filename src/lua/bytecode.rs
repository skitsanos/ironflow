...
pub(crate) fn sign(bytecode: &[u8]) -> String {
    let tag = crate::util::authentication::hmac_sha256(process_key(), bytecode);
...
pub(crate) fn verify(signed_b64: &str) -> Result<Vec<u8>> {
    ...
    let expected = crate::util::authentication::hmac_sha256(process_key(), bytecode);
    if !crate::util::authentication::constant_time_eq(tag, &expected) {
        anyhow::bail!(
