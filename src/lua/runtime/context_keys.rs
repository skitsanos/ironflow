...
///
/// Non-table values are converted as-is and rejected by `parse_context_keys`
/// with the same "must be an array of strings" wording as JSON configs.
...
pub(super) fn register_context_keys(lua: &Lua, builder: &LuaTable) -> LuaResult<()> {
    let context_keys = lua.create_function(|lua, (builder, keys): (LuaTable, LuaValue)| {
        ...
        if step.get::<String>("node_type")? != "code" {
            return Err(LuaError::RuntimeError(CONTEXT_KEYS_UNSUPPORTED.into()));
        }
