...
pub(super) fn register_flow_api(
    lua: &Lua,
    ...
    let constructor_diagnostics = diagnostics.clone();
    let new_fn = lua.create_function(move |lua, name: String| {
        ...
        let step_diagnostics = constructor_diagnostics.clone();
        let step_fn = lua.create_function(
            move |lua, (flow_tbl, step_name, node_arg): (LuaTable, String, LuaValue)| {
                ...
                let node_config: LuaTable = match node_arg {
                    ...
                    LuaValue::Function(func) => {
                        let b64 = serialize_handler(&func, step_diagnostics.as_ref())?;
        ...
        let step_if_diagnostics = constructor_diagnostics.clone();
        let step_if_fn = lua.create_function(
            move |lua,
                ...
                let node_config: LuaTable = match node_arg {
                    LuaValue::Table(tbl) => tbl,
                    LuaValue::Function(func) => {
                        let b64 = serialize_handler(&func, step_if_diagnostics.as_ref())?;
                        let tbl = lua.create_table()?;
                        tbl.set("_node_type", "code")?;
                        tbl.set("bytecode_b64", b64)?;
                        tbl
    ...
    register_node_factories(lua, registry, diagnostics)?;
