...
if ARGV[6] ~= '-1' and
   (string.match(ARGV[6], '^[1-9]%d*$') == nil or ttl == nil or ttl > 99999999999) then
...
local safety = tonumber(ARGV[14])
if string.match(ARGV[14], '^%d+$') == nil or safety == nil or safety > max_integer then
    return redis.error_reply('IRONFLOW_INVALID_RUN_LEASE_SAFETY')
end
if ARGV[11] ~= 'owner' and ARGV[11] ~= 'expired' then
...
-- Redis does not roll back earlier writes when a later command fails.
local seen = {}
for index, key in ipairs(KEYS) do
    if seen[key] then return redis.error_reply('IRONFLOW_STATE_KEY_ALIAS') end
    seen[key] = true
    local expected = index <= 2 and 'hash' or 'zset'
...
if string.match(expiry, '^%-?%d+$') == nil or expiry_number == nil or
   math.abs(expiry_number) > max_integer then
...
if ARGV[11] == 'owner' then
    if owner ~= ARGV[12] or expiry_number <= now then return -2 end
elseif ARGV[11] == 'expired' then
    if expiry_number > now then return -2 end
end
...
local applied_ttl = ttl
if ttl ~= -1 and ARGV[13] ~= '1' then
    local lease_window = expiry_number - now + safety
    if math.abs(lease_window) > max_integer then
        return redis.error_reply('IRONFLOW_INVALID_RUN_LEASE_TTL')
    end
    applied_ttl = math.max(math.ceil(lease_window / 1000000), ttl, 1)
end
...
if applied_ttl ~= -1 then redis.call('EXPIRE', KEYS[1], string.format('%.0f', applied_ttl))
