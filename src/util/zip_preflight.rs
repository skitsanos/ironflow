//! Shared bounded ZIP validation before `zip` allocates entry metadata.
...
pub(crate) use central_directory::Limits;
...
pub(super) struct Directory {
    ...
    pub(super) size: u64,
    pub(super) end: u64,
    pub(super) extra_metadata_bytes: u64,
}
...
/// Admit raw metadata before ZIP construction and return the raw entry count.
...
fn parse_directory<R: Read + Seek>(
    ...
    if !uses_zip64 {
        ...
        return Ok(Directory {
            ...
            extra_metadata_bytes: u64::from(le_u16(&eocd[20..22])),
        });
    }

    let mut directory = parse_zip64_directory(reader, eocd_offset, classic, operation)?;
    directory.extra_metadata_bytes = directory
        .extra_metadata_bytes
        .checked_add(u64::from(le_u16(&eocd[20..22])))
        .ok_or_else(|| anyhow::anyhow!("{operation}: ZIP end-record metadata overflow"))?;
    Ok(directory)
}
...
fn parse_zip64_directory<R: Read + Seek>(
    ...
    Ok(Directory {
        ...
        extra_metadata_bytes: record_size - 44,
    })
}
